...
<template>
  <div class="min-h-screen">
    <header class="flex justify-between items-center mb-10">
      ...
      <div class="flex items-center gap-4">
        ...
        <div class="flex items-center gap-3 pl-4 border-l border-slate-200 dark:border-slate-700">
          <img src="https://i.pravatar.cc/150?u=doctor" class="w-10 h-10 rounded-full object-cover border-2 border-white dark:border-slate-700" alt="Doctor" />
          <div class="hidden md:block">
            <h4 class="text-sm font-bold text-slate-800 dark:text-white">Dr. Evelyn Reed</h4>
            <p class="text-xs text-slate-500 dark:text-slate-400">Cardiologist</p>
          </div>
        </div>
      </div>
    </header>
    ...
    <div class="flex justify-between items-end mb-8">
      <div>
        <h1 class="text-3xl font-bold text-slate-900 dark:text-white mb-2">Welcome back, Dr. Reed</h1>
    ...
    <div class="bg-blue-50 dark:bg-blue-900/20 p-8 rounded-2xl border border-blue-100 dark:border-blue-800/50 mb-10 flex flex-col items-center justify-center text-center">
      <NuxtLink to="/consultation" class="bg-blue-600 hover:bg-blue-700 text-white px-8 py-3 rounded-full font-bold shadow-lg shadow-blue-500/30 flex items-center gap-2 transition transform active:scale-95">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
      <div v-for="patient in appointments" :key="patient.id" class="bg-white dark:bg-slate-800 p-5 rounded-2xl border border-slate-100 dark:border-slate-700 shadow-sm hover:shadow-md transition">
        ...
        <div class="grid grid-cols-2 gap-3">
          ...
          <button v-if="patient.time" class="px-4 py-2 bg-teal-600 text-white rounded-lg font-medium text-sm hover:bg-teal-700 shadow-sm transition">Start</button>
